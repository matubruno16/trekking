...
export const TarjetaFlotante = ({ contenido, onClose, visible }) => {
  return (
    <div className={`fixed inset-0 z-50 overflow-auto bg-black bg-opacity-60 flex justify-center items-center transition-opacity backdrop-blur-sm ${visible ? '' : 'opacity-0 pointer-events-none'}`}>
      {contenido && (
        <div className={`relative rounded-lg overflow-hidden shadow-lg p-6 mx-auto my-8 w-[90%] h-[70%] md:w-2/3 md:h-2/3 ${contenido.background} bg-no-repeat bg-cover bg-center inset-0 transform transition-transform shadow-gray-400 shadow-sm ${visible ? 'scale-100' : 'scale-0'}`}>
          <img
            src={close}
            width={60}
            alt="cerrar ventana"
